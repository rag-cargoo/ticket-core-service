...
> - Step 9: 결제/좌석 점유 상태머신(홀드/확정/만료)
> - Step 10: 취소/환불/재판매 대기열 연계
> - Step 11: 판매 정책 엔진(선예매/등급/1인 제한)
> - Step 12: 부정사용 방지/감사 추적
<!-- DOC_TOC_END -->
...
> 10. [Step 9: 결제/좌석 점유 상태머신(홀드/확정/만료)](#step-9-결제좌석-점유-상태머신홀드확정만료)
> 11. [Step 10: 취소/환불/재판매 대기열 연계](#step-10-취소환불재판매-대기열-연계)
> 12. [Step 11: 판매 정책 엔진(선예매등급1인-제한)](#step-11-판매-정책-엔진선예매등급1인-제한)
> 13. [Step 12: 부정사용 방지감사 추적](#step-12-부정사용-방지감사-추적)
...
>
> ### 8.7. Execution Log - Before (Raw Evidence, 2026-02-11)
> ```bash
> cd workspace/apps/backend/ticket-core-service
...
> - http_req_duration.avg: 2.593ms
> - http_req_duration.p(95): 3.848ms
> - http_req_duration.p(99): 5.405ms
> - join_accepted.count: 400
> - join_rejected.count: 57667
> ```
>
> ### 8.8. 병목 제거 Change Set (코드 반영)
...
>   - 동일 사용자의 재요청 시 기존 rank를 반환해 불필요한 재정렬/재조회 경로를 제거.
> - `src/main/java/com/ticketrush/api/waitingqueue/WaitingQueueController.java`
>   - `join/status/subscribe` 요청 로그를 `info -> debug`로 조정해 핫패스 로그 I/O 비용 완화.
> - 회귀 검증:
>   - `bash ./scripts/api/run-step7-regression.sh` PASS (최적화 반영 후 SSE 계약 회귀 없음).
>
> ### 8.9. Execution Log - After (Raw Evidence, 2026-02-11)
> ```bash
> cd workspace/apps/backend/ticket-core-service
> K6_VUS=20 K6_DURATION=300s make test-k6
> ```
...
> - http_req_failed.rate: 0
> - http_req_duration.avg: 2.121ms
> - http_req_duration.p(95): 3.552ms
> - http_req_duration.p(99): 4.810ms
> - join_accepted.count: 400
> - join_rejected.count: 57960
> ```
>
> ### 8.10. Before & After (동일 조건 수치 비교)
> | Metric | Before | After | Delta |
> | :--- | :--- | :--- | :--- |
> | http_reqs.count | 58067 | 58360 | +293 (+0.50%) |
> | http_reqs.rate | 193.464 req/s | 194.410 req/s | +0.945 req/s (+0.49%) |
> | http_req_duration.avg | 2.593ms | 2.121ms | -0.472ms (-18.22%) |
> | http_req_duration.p(95) | 3.848ms | 3.552ms | -0.295ms (-7.68%) |
> | http_req_duration.p(99) | 5.405ms | 4.810ms | -0.595ms (-11.01%) |
> | http_req_failed.rate | 0 | 0 | 변화 없음 |
>
> ### 8.10-1. 테스트 검증 설명
> Step 8 검증은 `K6_VUS=20`, `K6_DURATION=300s` 동일 조건에서 Before(`prj-docs/api-test/k6-before-step8.md`, `prj-docs/api-test/k6-summary-before-step8.json`)와 After(`prj-docs/api-test/k6-latest.md`, `prj-docs/api-test/k6-summary.json`)를 비교한다. 판독은 `http_req_failed.rate`가 `0 -> 0`으로 유지되는지로 안정성을 먼저 확인하고, `http_req_duration.p(95)`, `http_req_duration.p(99)`가 감소했는지로 tail latency 개선 여부를 확인한 뒤, `http_reqs.rate` 유지/상승 여부와 `join_accepted.count=400` 유지 여부로 처리량과 throttling 계약 보존을 함께 확인한다. 이번 결과는 실패율 증가 없이 `p95/p99`를 줄이고 처리량을 소폭 높였으므로 병목 제거 개선으로 판정한다.
>
> ### 8.11. 완료 판정 (Step 8)
> - 코드 병목 제거(1차) + 동일 조건 재측정 + before/after 수치 증빙을 충족했으므로 Step 8을 완료로 판정한다.
> - 증빙 파일:
>   - `prj-docs/api-test/k6-before-step8.md`
>   - `prj-docs/api-test/k6-summary-before-step8.json`
>   - `prj-docs/api-test/k6-latest.md`
>   - `prj-docs/api-test/k6-summary.json`
>
> ### 8.12. 다음 개선 액션 (Step 9 준비)
> - `status/subscribe` 혼합 부하 시나리오를 k6에 확장해 Step 9 기준선으로 승격한다.
> - 스케줄러 fan-out 경로에 대한 emitter 수/지연 계측 포인트를 추가한다.
...
> [!TIP]
> **작업 전 이해 가이드**: Step 9는 "예약 요청 1회 처리"를 넘어서 결제 대기 시간을 모델링하는 단계다. 좌석을 즉시 확정하지 않고 `HOLD -> PAYING -> CONFIRMED|EXPIRED`로 상태를 분리해 결제 지연/이탈/중복 요청을 통제한다.
>
> ### 9.1. Failure-First: 상태머신 없이 구현할 때 실패하는 이유
> - **함정 1 (즉시 확정)**: 결제 완료 전 좌석을 `RESERVED`로 확정하면 결제 실패 시 좌석 회수가 어렵다.
> - **함정 2 (TTL 부재)**: 홀드 만료 시간이 없으면 좌석이 영구 점유되어 실제 판매량이 줄어든다.
> - **함정 3 (전이 규칙 누락)**: `HOLD` 없이 바로 `CONFIRMED`로 전이되면 중간 상태 재시도 정책이 깨진다.
>
> ### 9.2. 상태 전이 다이어그램
> ```text
> AVAILABLE
>   -> HOLD (seat=TEMP_RESERVED, holdExpiresAt 설정)
>   -> PAYING (결제 진행)
>   -> CONFIRMED (seat=RESERVED)
>
> HOLD/PAYING --(TTL 만료)--> EXPIRED (seat=AVAILABLE 복구)
> ```
>
> ### 9.3. 구현 반영 (Change Set)
> - 상태머신 도메인:
>   - `src/main/java/com/ticketrush/domain/reservation/entity/Reservation.java`
>   - 상태값 확장: `HOLD`, `PAYING`, `CONFIRMED`, `EXPIRED`
>   - 전이 메서드: `startPaying`, `confirmPayment`, `expire`
> - 홀드/결제 라이프사이클 서비스:
>   - `src/main/java/com/ticketrush/domain/reservation/service/ReservationLifecycleServiceImpl.java`
> - 만료 스케줄러:
>   - `src/main/java/com/ticketrush/global/scheduler/ReservationLifecycleScheduler.java`
> - API(v6):
>   - `POST /api/reservations/v6/holds`
>   - `POST /api/reservations/v6/{reservationId}/paying`
>   - `POST /api/reservations/v6/{reservationId}/confirm`
>   - `GET /api/reservations/v6/{reservationId}?userId=...`
>
> ### 9.4. 검증 근거 (Execution Evidence)
> - 단위 테스트:
>   - `ReservationStateMachineTest` (상태 전이 규칙 검증)
> - 통합 테스트:
>   - `ReservationLifecycleServiceIntegrationTest` (홀드 생성/확정, TTL 만료 시 EXPIRED+좌석 복구 검증)
> - 스케줄러 테스트:
>   - `ReservationLifecycleSchedulerTest` (만료 스캔 위임 검증)
> - 실행 명령:
>   - `./gradlew test --tests '*ReservationStateMachineTest' --tests '*ReservationLifecycleServiceIntegrationTest' --tests '*ReservationLifecycleSchedulerTest'`
> - 결과:
>   - `7 tests completed, 0 failed`
> - 실행 리포트:
>   - `prj-docs/api-test/step9-lifecycle-latest.md`
>
> ### 9.4-A. Before/After 핵심 비교 (Step 8 -> Step 9)
> | 검증 항목 | Before (Step 8) | After (Step 9) | 확인 위치 |
> | --- | --- | --- | --- |
> | 예약 상태 모델 | `PENDING, CONFIRMED, CANCELLED` 중심 | `HOLD, PAYING, CONFIRMED, EXPIRED` 포함 상태머신 | `src/main/java/com/ticketrush/domain/reservation/entity/Reservation.java` |
> | 결제 대기 모델링 | 없음 (즉시 확정/취소 중심) | `holdExpiresAt` 기반 결제 대기 + 만료 복구 | `src/main/java/com/ticketrush/domain/reservation/service/ReservationLifecycleServiceImpl.java` |
> | 만료 처리 방식 | 수동/부재 | 스케줄러 기반 자동 만료 스캔 및 복구 | `src/main/java/com/ticketrush/global/scheduler/ReservationLifecycleScheduler.java` |
> | API 흐름 | 상태 전이 전용 API 부재 | `v6/holds -> v6/{id}/paying -> v6/{id}/confirm -> v6/{id}` | `src/main/java/com/ticketrush/api/controller/ReservationController.java` |
> | 실행 증거 | 문서형 기준만 존재 | 단위/통합/스케줄러 테스트 + API E2E PASS 로그 | `prj-docs/api-test/step9-lifecycle-latest.md` |
>
> ### 9.4-B. 테스트 검증 설명 (로그를 어떻게 읽는지)
> - 코드 테스트 판정:
>   - `./gradlew test --tests '*ReservationStateMachineTest' --tests '*ReservationLifecycleServiceIntegrationTest' --tests '*ReservationLifecycleSchedulerTest'` 실행 후 `7 tests completed, 0 failed` 이면 상태 전이/만료/스케줄러 위임 검증이 통과한 것이다.
> - API E2E 판정:
>   - `.codex/tmp/ticket-core-service/step9/20260211-213008-e2e/e2e-status.txt` 값이 `health_check_ok`이면 기동 상태 검증이 통과한 것이다.
>   - `.codex/tmp/ticket-core-service/step9/20260211-213008-e2e/v8-step9-e2e.log`에서 `status=HOLD`, `status=PAYING`, `status=CONFIRMED`, `검증 종료 (PASS)`가 순서대로 나오면 라이프사이클 API 흐름이 정상 완료된 것이다.
> - 최종 완료 판정:
>   - 코드(상태머신/만료 처리) + 테스트(단위/통합/스케줄러) + 운영 스크립트(`v8-reservation-lifecycle.sh`) + 문서 동기화가 모두 충족되면 Step 9 완료로 본다.
>
> ### 9.5. 운영 스크립트/문서 동기화
> - API 명세: `prj-docs/api-specs/reservation-api.md`
> - HTTP 샘플: `scripts/http/reservation.http`
> - 회귀 스크립트: `scripts/api/v8-reservation-lifecycle.sh`
>
> ### 9.6. 완료 판정 (Step 9)
> - 상태머신 코드 + TTL 만료 처리 + API(v6) + 테스트 + 문서/스크립트 동기화를 충족했으므로 Step 9를 완료로 판정한다.
...
> [!TIP]
> **작업 전 이해 가이드**: Step 10은 "취소하면 좌석만 비우는 처리"를 넘어서, `취소 -> 재판매 대기열 승격 -> 환불 완료`까지 한 트랜잭션 흐름으로 묶어 운영 정합성을 보장하는 단계다.
>
> ### 10.1. Failure-First: 취소/환불/재판매 연계를 분리하면 실패하는 이유
> - **함정 1 (취소만 처리)**: `CONFIRMED -> CANCELLED`만 반영하고 대기열 승격을 누락하면 좌석이 풀려도 구매 기회가 즉시 전달되지 않는다.
> - **함정 2 (환불 상태 미기록)**: 취소 이후 환불 완료 시점을 기록하지 않으면 CS/정산 관점에서 예약 상태 추적이 끊긴다.
> - **함정 3 (API 분리 기준 부재)**: 취소/환불 전이를 동일 엔드포인트로 뭉개면 중복 요청/재시도 시 멱등성 판단이 어려워진다.
>
> ### 10.2. 상태 전이 다이어그램
> ```text
> CONFIRMED
>   -> CANCELLED (seat=AVAILABLE 복구)
>   -> REFUNDED (환불 완료 시각 기록)
>
> CANCELLED --(resale linkage)--> waiting-queue top 1 ACTIVE
> ```
>
> ### 10.3. 구현 반영 (Change Set)
> - 상태머신 도메인:
>   - `src/main/java/com/ticketrush/domain/reservation/entity/Reservation.java`
>   - 상태값/필드 확장: `REFUNDED`, `cancelledAt`, `refundedAt`
>   - 전이 메서드: `cancel`, `refund`
> - 취소/환불 + 대기열 연계 서비스:
>   - `src/main/java/com/ticketrush/domain/reservation/service/ReservationLifecycleServiceImpl.java`
>   - 취소 시 `activateUsers(concertId, 1)` 호출 + SSE `ACTIVE` 이벤트 전송
> - API(v6):
>   - `POST /api/reservations/v6/{reservationId}/cancel`
>   - `POST /api/reservations/v6/{reservationId}/refund`
> - 응답 확장:
>   - `src/main/java/com/ticketrush/api/dto/reservation/ReservationLifecycleResponse.java`
>   - `resaleActivatedUserIds` 포함
>
> ### 10.4. 검증 근거 (Execution Evidence)
> - 단위 테스트:
>   - `ReservationStateMachineTest` (취소/환불 전이 규칙 검증 포함)
> - 통합 테스트:
>   - `ReservationLifecycleServiceIntegrationTest` (취소 시 좌석 복구 + 대기열 승격 + 환불 상태 검증)
> - 스케줄러 테스트:
>   - `ReservationLifecycleSchedulerTest`, `WaitingQueueSchedulerTest`
> - 실행 명령:
>   - `./gradlew test --tests '*ReservationStateMachineTest' --tests '*ReservationLifecycleServiceIntegrationTest' --tests '*ReservationLifecycleSchedulerTest' --tests '*WaitingQueueSchedulerTest'`
> - 결과:
>   - `13 tests completed, 0 failed`
> - API E2E:
>   - `bash scripts/api/v9-cancel-refund-resale.sh`
>   - run-id: `20260211-220150-e2e`
>   - `WAITING -> ACTIVE`, `CANCELLED`, `REFUNDED` 확인
> - 실행 리포트:
>   - `prj-docs/api-test/step10-cancel-refund-latest.md`
>
> ### 10.4-A. Before/After 핵심 비교 (Step 9 -> Step 10)
> | 검증 항목 | Before (Step 9) | After (Step 10) | 확인 위치 |
> | --- | --- | --- | --- |
> | 취소/환불 전이 | `CANCELLED`는 확장 전이 없음 | `CONFIRMED -> CANCELLED -> REFUNDED` 명시적 전이 | `src/main/java/com/ticketrush/domain/reservation/entity/Reservation.java` |
> | 재판매 대기열 연계 | 취소 시 대기열 승격 없음 | 취소 시 대기열 상위 1명 `ACTIVE` 승격 | `src/main/java/com/ticketrush/domain/reservation/service/ReservationLifecycleServiceImpl.java` |
> | API 계약 | Step9 API만 존재 (`holds/paying/confirm/get`) | `cancel`, `refund` API 추가 | `src/main/java/com/ticketrush/api/controller/ReservationController.java` |
> | 운영 검증 | Step9 E2E만 존재 | Step10 E2E(`v9`) 추가 | `scripts/api/v9-cancel-refund-resale.sh` |
>
> ### 10.4-B. 테스트 검증 설명 (로그를 어떻게 읽는지)
> - 코드 테스트 판정:
>   - JUnit XML에서 `ReservationStateMachineTest tests=6 failures=0`, `ReservationLifecycleServiceIntegrationTest tests=4 failures=0`이면 취소/환불 규칙과 서비스 연계가 통과한 것이다.
> - API E2E 판정:
>   - `.codex/tmp/ticket-core-service/step10/20260211-220150-e2e/e2e-status.txt` 값이 `health_check_ok`이면 기동 검증이 통과한 것이다.
>   - `v9-step10-e2e.log`에서 `status=WAITING`, `activatedUser=...`, `status=ACTIVE`, `status=REFUNDED`, `검증 종료 (PASS)`가 순서대로 나오면 취소/재판매/환불 전체 플로우가 정상 완료된 것이다.
>
> ### 10.5. 운영 스크립트/문서 동기화
> - API 명세: `prj-docs/api-specs/reservation-api.md`
> - HTTP 샘플: `scripts/http/reservation.http`
> - 회귀 스크립트: `scripts/api/v9-cancel-refund-resale.sh`
> - 실행 리포트: `prj-docs/api-test/step10-cancel-refund-latest.md`
>
> ### 10.6. 완료 판정 (Step 10)
> - 취소/환불 상태 전이 + 대기열 승격 + 테스트(단위/통합/스케줄러) + API E2E + 문서/스크립트 동기화를 충족했으므로 Step 10을 완료로 판정한다.
...
> [!TIP]
> **작업 전 이해 가이드**: Step 11은 "좌석을 선점할 수 있는 타이밍/대상/개수"를 하드코딩이 아닌 정책 데이터로 통제하는 단계다. 예약 API를 바꾸지 않고도 운영자가 콘서트별 판매 규칙을 바꿀 수 있어야 한다.
>
> ### 11.1. Failure-First: 정책 엔진 없이 구현할 때 실패하는 이유
> - **함정 1 (오픈 시간 하드코딩)**: 일반 오픈 시간을 코드 상수로 박아두면 운영 변경 시 재배포가 필요하다.
> - **함정 2 (등급 검증 부재)**: 선예매 구간에서 등급 검증이 없으면 일반 회원도 조기 진입 가능해 정책 위반이 발생한다.
> - **함정 3 (1인 제한 부재)**: 동일 유저가 여러 좌석을 동시에 `HOLD`하면 실질적인 매크로 점유와 같은 결과가 난다.
>
> ### 11.2. 정책 평가 흐름
> ```text
> createHold(user, seat) 진입
>   -> concertId 기준 sales_policy 조회
>   -> (선예매 창) user.tier >= presaleMinimumTier 검증
>   -> (선예매 창 외) now >= generalSaleStartAt 검증
>   -> user+concert 기준 HOLD/PAYING/CONFIRMED 개수 조회
>   -> maxReservationsPerUser 초과 시 차단
>   -> 통과 시에만 seat.hold() 실행
> ```
>
> ### 11.3. 구현 반영 (Change Set)
> - 정책 도메인:
>   - `src/main/java/com/ticketrush/domain/reservation/entity/SalesPolicy.java`
>   - 필드: `presaleStartAt`, `presaleEndAt`, `presaleMinimumTier`, `generalSaleStartAt`, `maxReservationsPerUser`
> - 정책 서비스:
>   - `src/main/java/com/ticketrush/domain/reservation/service/SalesPolicyServiceImpl.java`
>   - 역할: 정책 upsert/get + HOLD 요청 검증
> - 예약 연동:
>   - `src/main/java/com/ticketrush/domain/reservation/service/ReservationLifecycleServiceImpl.java`
>   - `createHold`에서 `salesPolicyService.validateHoldRequest(...)` 선검증 추가
> - API:
>   - `PUT /api/concerts/{concertId}/sales-policy`
>   - `GET /api/concerts/{concertId}/sales-policy`
>   - `POST /api/users`에 `tier` 필드 확장(미입력 시 `BASIC`)
>
> ### 11.4. 검증 근거 (Execution Evidence)
> - 통합 테스트:
>   - `ReservationLifecycleServiceIntegrationTest`
>   - 추가 케이스:
>     - `holdInPresale_shouldRejectUserWithInsufficientTier`
>     - `holdShouldEnforcePerUserReservationLimit`
> - 실행 명령:
>   - `./gradlew test --tests '*ReservationStateMachineTest' --tests '*ReservationLifecycleServiceIntegrationTest' --tests '*ReservationLifecycleSchedulerTest'`
> - 결과:
>   - `BUILD SUCCESSFUL`
...
>   - 기대 판정:
>     - BASIC 유저 HOLD: `409 Presale tier not eligible`
>     - VIP 유저 첫 HOLD: `201 HOLD`
>     - VIP 유저 두 번째 HOLD: `409 Per-user reservation limit exceeded`
>
> ### 11.4-A. Before/After 핵심 비교 (Step 10 -> Step 11)
> | 검증 항목 | Before (Step 10) | After (Step 11) | 확인 위치 |
> | --- | --- | --- | --- |
> | 판매 정책 저장소 | 없음 | `sales_policies` 테이블 기반 정책 모델 | `src/main/java/com/ticketrush/domain/reservation/entity/SalesPolicy.java` |
> | 선예매 등급 제어 | 미지원 | `presaleMinimumTier` 검증으로 선예매 접근 통제 | `src/main/java/com/ticketrush/domain/reservation/service/SalesPolicyServiceImpl.java` |
> | 1인 예약 제한 | 미지원 | `HOLD/PAYING/CONFIRMED` 합산 기반 제한 적용 | `src/main/java/com/ticketrush/domain/reservation/repository/ReservationRepository.java` |
> | 운영 조정 방식 | 코드 수정 필요 | API로 정책 갱신 가능 (`PUT /concerts/{id}/sales-policy`) | `src/main/java/com/ticketrush/api/controller/ConcertController.java` |
> | 회귀 자동화 | Step10까지 스크립트 존재 | Step11 정책 시나리오 스크립트 추가 | `scripts/api/v10-sales-policy-engine.sh` |
>
> ### 11.4-B. 테스트 검증 설명 (로그를 어떻게 읽는지)
> - 코드 테스트 판정:
>   - `ReservationLifecycleServiceIntegrationTest`에서 Step11 케이스 2개가 통과하면 등급 차단과 1인 제한 로직이 서비스 계층에서 동작한 것이다.
> - API E2E 판정:
>   - `v10-sales-policy-engine.sh` 로그에서 `Step 3(기대:409)`, `Step 4(기대:HOLD)`, `Step 5(기대:409)`가 모두 성공이어야 정책 엔진이 실제 API 체인에 반영된 것이다.
>
> ### 11.5. 운영 스크립트/문서 동기화
> - API 명세: `prj-docs/api-specs/concert-api.md`, `prj-docs/api-specs/reservation-api.md`, `prj-docs/api-specs/user-api.md`
> - HTTP 샘플: `scripts/http/reservation.http`
> - 회귀 스크립트: `scripts/api/v10-sales-policy-engine.sh`
>
> ### 11.6. 완료 판정 (Step 11)
> - 정책 테이블 + 예약 전 정책 검증 + 등급/1인 제한 테스트 + API E2E 스크립트 + 문서 동기화를 충족하면 Step 11 완료로 판정한다.
...
> [!TIP]
> **작업 전 이해 가이드**: Step 12는 "요청이 들어오면 처리한다"에서 끝나지 않고, 비정상 패턴을 실시간 차단하고 사후 감사 가능한 형태로 흔적을 남기는 단계다.
>
> ### 12.1. Failure-First: 차단 규칙/감사 로그가 없을 때 실패하는 이유
> - **함정 1 (빈도 통제 부재)**: 같은 사용자가 짧은 시간에 HOLD를 반복 호출하면 좌석 점유 편향이 발생한다.
> - **함정 2 (중복 요청 식별 부재)**: 네트워크 재시도나 봇 재전송이 같은 요청을 반복해도 구분하지 못한다.
> - **함정 3 (디바이스 다계정 탐지 부재)**: 동일 디바이스가 여러 계정으로 우회 진입해도 운영자가 사후 확인할 근거가 없다.
> - **함정 4 (트랜잭션 롤백 시 감사 유실)**: 차단 예외 발생 시 로그 저장이 같은 트랜잭션에 묶이면 함께 롤백되어 증거가 사라진다.
>
> ### 12.2. 검증/기록 흐름
> ```text
> createHold(user, seat, requestFingerprint, deviceFingerprint)
>   -> rate-limit 검사
>   -> duplicate requestFingerprint 검사
>   -> deviceFingerprint 다계정 검사
>   -> 차단 시: BLOCKED 로그를 REQUIRES_NEW로 커밋 후 409 반환
>   -> 통과 시: HOLD 생성 후 ALLOWED 로그 기록
>   -> 운영 조회 API(/v6/audit/abuse)로 필터 조회
> ```
>
> ### 12.3. 구현 반영 (Change Set)
> - 감사 로그 모델:
>   - `src/main/java/com/ticketrush/domain/reservation/entity/AbuseAuditLog.java`
>   - 필드: `action`, `result`, `reason`, `requestFingerprint`, `deviceFingerprint`, `occurredAt`
> - 차단/조회 서비스:
>   - `src/main/java/com/ticketrush/domain/reservation/service/AbuseAuditServiceImpl.java`
>   - 룰: `RATE_LIMIT_EXCEEDED`, `DUPLICATE_REQUEST_FINGERPRINT`, `DEVICE_FINGERPRINT_MULTI_ACCOUNT`
> - 차단 로그 보존 트랜잭션:
>   - `src/main/java/com/ticketrush/domain/reservation/service/AbuseAuditWriter.java`
>   - `saveBlocked`는 `REQUIRES_NEW`로 커밋
> - 예약 연동:
>   - `src/main/java/com/ticketrush/domain/reservation/service/ReservationLifecycleServiceImpl.java`
>   - HOLD 생성 전 `validateHoldRequest` 호출 + 성공 시 `recordAllowedHold`
> - 운영 조회 API:
>   - `GET /api/reservations/v6/audit/abuse`
>   - 구현: `src/main/java/com/ticketrush/api/controller/ReservationController.java`
>
> ### 12.4. 검증 근거 (Execution Evidence)
> - 통합 테스트:
>   - `ReservationLifecycleServiceIntegrationTest`
>   - 추가 케이스:
>     - `holdShouldBlockWhenRateLimitExceeded`
>     - `holdShouldBlockWhenRequestFingerprintIsDuplicated`
>     - `holdShouldBlockWhenDeviceFingerprintIsSharedAcrossAccounts`
>     - `abuseAuditQuery_shouldReturnBlockedEvent`
> - 실행 명령:
>   - `./gradlew test --rerun-tasks --tests '*ReservationStateMachineTest' --tests '*ReservationLifecycleServiceIntegrationTest' --tests '*ReservationLifecycleSchedulerTest'`
> - 결과:
>   - `BUILD SUCCESSFUL`
> - API E2E:
>   - `bash scripts/api/v11-abuse-audit.sh`
>   - 기대 판정:
>     - rate-limit: 4번째 요청 `409`
>     - duplicate fingerprint: 동일 fingerprint 재호출 `409`
>     - multi-account device: 2번째 계정 진입 `409`
>     - 감사 조회 API에서 차단 사유 3종 조회
>
> ### 12.4-A. Before/After 핵심 비교 (Step 11 -> Step 12)
> | 검증 항목 | Before (Step 11) | After (Step 12) | 확인 위치 |
> | --- | --- | --- | --- |
> | 부정사용 차단 규칙 | 없음 | `rate`, `duplicate`, `device` 3종 차단 | `src/main/java/com/ticketrush/domain/reservation/service/AbuseAuditServiceImpl.java` |
> | 감사 로그 모델 | 없음 | `abuse_audit_logs` + 사유 코드 저장 | `src/main/java/com/ticketrush/domain/reservation/entity/AbuseAuditLog.java` |
> | 차단 로그 영속성 | 예외 시 로그 롤백 가능 | `REQUIRES_NEW`로 차단 로그 보존 | `src/main/java/com/ticketrush/domain/reservation/service/AbuseAuditWriter.java` |
> | 운영 조회 | 없음 | `GET /api/reservations/v6/audit/abuse` 필터 조회 | `src/main/java/com/ticketrush/api/controller/ReservationController.java` |
> | 운영 검증 | Step11(`v10`)까지 | Step12 E2E(`v11`) 추가 | `scripts/api/v11-abuse-audit.sh` |
>
> ### 12.4-B. 테스트 검증 설명 (로그를 어떻게 읽는지)
> - 코드 테스트 판정:
>   - `ReservationLifecycleServiceIntegrationTest`에서 Step12 케이스 4개가 모두 통과하면 차단 규칙+감사 조회가 서비스 단에서 동작하는 것이다.
> - API E2E 판정:
>   - `.codex/tmp/ticket-core-service/step12/20260212-010759-e2e/e2e-status.txt` 값이 `health_check_ok`이면 기동 검증이 통과한 것이다.
>   - `v11-step12-e2e.log`에서 `4th blocked: 409`, `duplicate blocked: 409`, `multi-account blocked: 409`, `차단 사유 3종 조회됨`이 모두 나오면 Step12 핵심 요구사항이 충족된 것이다.
>
> ### 12.5. 운영 스크립트/문서 동기화
> - API 명세: `prj-docs/api-specs/reservation-api.md`
> - HTTP 샘플: `scripts/http/reservation.http`
> - 회귀 스크립트: `scripts/api/v11-abuse-audit.sh`
> - 실행 리포트: `prj-docs/api-test/step12-abuse-audit-latest.md`
>
> ### 12.6. 완료 판정 (Step 12)
> - 차단 규칙 3종 + 감사 로그 영속성 + 운영 조회 API + 테스트/E2E/문서 동기화를 충족했으므로 Step 12를 완료로 판정한다.
