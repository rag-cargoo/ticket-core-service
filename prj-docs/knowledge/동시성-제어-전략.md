...
> ```
>
> ### 1.4. 실험 결과 및 분석 (Experimental Results)
> ---
>   - **성공 횟수**: 1건
>   - **실패 횟수**: 29건 (ObjectOptimisticLockingFailureException 발생)
>   - **실제 처리 로그 (Raw Log)**:
>     ```bash
>     # [v1] 낙관적 락 예약 API 호출 결과
>     HTTP/1.1 200 OK
>     {"id":2, "userId":9, "seatId":2, "reservationTime":"2026-02-06T17:22:30..."}
>     >> [SUCCESS] 낙관적 락 정합성 검증 완료
>     ```
...
> ```
>
> ### 2.4. 결론 및 실측 데이터 (Conclusion)
> ---
>   - **장점**: 낙관적 락보다 정합성이 강력하고, 불필요한 예외 재시도 로직이 필요 없다.
>   - **단점**: 대기 시간이 길어지면 DB 전체 성능에 영향을 준다.
>   - **실제 처리 로그 (Raw Log)**:
>     ```bash
>     # [v2] 비관적 락 예약 API 호출 결과
>     HTTP/1.1 200 OK
>     {"id":3, "userId":10, "seatId":3, "reservationTime":"..."}
>     >> [SUCCESS] 비관적 락 순차 처리 검증 완료
>     ```
...
>
> #### 3.2.4. 실험 결과 및 실측 데이터 (Experimental Results)
> ---
>   - **성공 횟수**: 1건
>   - **실패 횟수**: 29건
...
>   - **실제 처리 로그 (Raw Log)**:
>     ```bash
>     # [v3] Redis 분산 락 예약 API 호출 결과
>     HTTP/1.1 200 OK
>     {"id":4, "userId":11, "seatId":4, "reservationTime":"..."}
>     >> [SUCCESS] 분산 락 트랜잭션 원자성 검증 완료
>     ```
>
> #### 3.2.5. 핵심 동작 원리
