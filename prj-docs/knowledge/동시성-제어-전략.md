...
>
> ### 6.1. 배경: 대기열 우회 공격 (Bypass Trap)
> ---
>   - **현상**: Step 5에서 프론트엔드 대기열을 구현했으나, 사용자가 예약 API(/api/v4/queue) 주소를 알아내어 직접 호출할 경우 대기열 로직이 무력화됨.
>   - **위험**: 줄을 서지 않은 수만 명의 사용자가 동시에 DB에 접근하여 커넥션 풀을 고갈시키고 시스템 전체를 마비시킴.
>
> ### 6.2. 코드 대조 (Before & After)
> ---
>
>   - **Before: 검증 없는 API**
>     - 어떤 사용자든 API 호출 시 즉시 Kafka 프로듀서 로직이 실행되어 시스템 부하 가중.
>
>   - **After: 인터셉터 진입 가드 적용**
>     ```java
>     // WaitingQueueInterceptor.java
>     @Override
>     public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
>         String userId = request.getHeader("User-Id");
>         // Redis에서 활성 유저 여부 0.1ms 내 확인
>         if (!redisTemplate.hasKey("active-user:" + userId)) {
>             response.sendError(403, "Not an active user in waiting queue");
>             return false; // 비즈니스 로직 진입 전 원천 차단
>         }
>         return true;
>     }
>     ```
>
> ### 6.3. 핵심 제어 매커니즘 (Control Mechanisms)
> ---
>   - **진입 차단 (Throttling)**: `ZCARD`로 대기열 인원 체크 후 `max-queue-size` 초과 시 즉시 `REJECTED`.
>   - **유입량 동적 조절**: `application.yml` 설정을 통해 활성화 배치 사이즈(Batch)와 토큰 유효 시간(TTL)을 실시간 튜닝.
>
> ### 6.4. 검증 결과 (Experimental Results)
> ---
>   - **테스트 시나리오**: 활성 토큰이 없는 유저 100명이 예약 API 직접 호출 시도.
>   - **결과**: 100명 전원 `403 Forbidden` 응답 확인 및 백엔드 로직 진입 0건 달성.
