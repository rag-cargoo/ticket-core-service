...
단순한 락(Lock) 방식은 트래픽이 몰릴 때 모든 DB 커넥션을 점유하여 시스템 전체를 마비시킵니다. 이를 해결하기 위해 요청을 즉시 처리하지 않고 **Kafka**라는 완충 지대에 담아 서버가 감당 가능한 속도로 처리하는 방식을 도입했습니다.

#### 데이터 흐름
1. **Producer**: 사용자의 예약 요청을 `ticket-reservation-events` 토픽으로 전송 (상태: `PENDING`)
2. **Kafka**: 요청 메시지를 파티션별로 순차적으로 저장
3. **Consumer**: 메시지를 꺼내어 실제 DB 예약 로직 수행 (상태: `PROCESSING` -> `SUCCESS/FAIL`)
4. **Notification**: SSE(Server-Sent Events) 또는 Polling을 통해 사용자에게 최종 결과 실시간 통보
...
- **이유**: 동일 좌석에 대한 모든 요청이 같은 파티션으로 들어가게 되어, 컨슈머가 이를 물리적으로 순차 처리(Serial Processing)할 수 있게 됨으로써 데이터 정합성을 완벽히 보호합니다.

#### 2.2. 상태 관리 (State Management)
비동기 환경에서 사용자는 자신의 요청 상태를 실시간으로 확인해야 합니다.
- **도구**: Redis (`StringRedisTemplate`)
- **키 구조**: `reservation:status:{userId}:{seatId}`
- **TTL**: 30분 (불필요한 메모리 점유 방지 및 자동 정리)

---

### 3. 트러블슈팅 기록 (Engineering Log)

#### 이슈: JSON 객체 직렬화 오류 (`ClassCastException`)
- **상황**: `ReservationEvent` 객체를 전송하려 했으나 기본 `StringSerializer` 설정으로 인해 에러 발생.
- **해결**: `application.yml` 설정을 통해 JSON 기반의 직렬화/역직렬화 체계를 전역적으로 강제함.
