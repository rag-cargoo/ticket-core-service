...
> ```
>
> ### 6.3. 검증 결과 및 실행 로그 (Experimental Results)
> ---
>   - **테스트 도구**: `scripts/api/v6-throttling-test.sh`
>   - **시나리오**: 활성 토큰이 없는 유저(ID: 999)가 예약 API를 직접 호출
>
> #### 실제 실행 로그 (Raw Log)
> ```bash
> >>>> [Step 6 Test] 유입량 제어 검증 시작...
> 1. 비활성 유저(999)의 예약 API 호출...
...
>   "path": "/api/v4/queue"
> }
> [SUCCESS] 비활성 유저 차단 완료 (Status: 403)
> ```
>
> ### 6.4. 핵심 교훈 (Lessons Learned)
> ---
