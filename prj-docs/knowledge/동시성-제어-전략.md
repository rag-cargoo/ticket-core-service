...
public class RedissonLockFacade {
    ...
    public ReservationResponse createReservation(ReservationRequest request) {
        // 1. 좌석별로 유니크한 락 키 생성 (ex: lock:seat:1)
        String lockKey = "lock:seat:" + request.seatId();
        RLock lock = redissonClient.getLock(lockKey);

        try {
            // 2. 락 획득 시도 (최대 10초 대기, 락 획득 후 2초간 점유)
            ...
            // 3. 실제 트랜잭션 로직 실행 (외부 Service 호출)
            // 핵심: Service의 @Transactional 메서드가 완전히 종료(Commit)된 후 아래로 내려감
            return reservationService.createReservation(request);

        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        } finally {
            // 4. DB 저장이 완벽히 끝난 후 안전하게 락 해제
...
> **Next Step**: Phase 3 - Kafka 기반 비동기 대기열 시스템 설계 및 구현 시작.