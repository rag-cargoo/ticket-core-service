...
>
> ### 8.10-1. 초보자 판독 가이드 (이 표를 어떻게 읽는가)
> - **1순위 확인값: 실패율**
>   - `http_req_failed.rate`가 `0`을 유지하면 "빨라졌지만 실패가 늘었다"는 역효과를 피한 것이다.
> - **2순위 확인값: p95/p99**
>   - 평균보다 `p95/p99`를 우선 본다.
>   - 이유: 실제 장애 체감은 느린 꼬리 구간(tail latency)에서 먼저 드러난다.
> - **3순위 확인값: 처리량**
>   - `http_reqs.rate`가 유지/상승하면 같은 시간에 더 많은 요청을 처리한 것이다.
> - **정책값 확인: accepted/rejected**
>   - Step 6 throttling 정책상 `join_accepted.count=400`은 정상 목표치다.
>   - `join_rejected.count` 증가는 과부하 차단 정책 결과이므로, 단독으로 성능 악화로 해석하지 않는다.
>
> ### 8.10-2. 이번 결과를 한 문장으로 해석
> - 실패율 `0`을 유지하면서 `p95 -7.68%`, `p99 -11.01%`, 처리량 `+0.49%`를 달성했으므로, **안정성을 해치지 않고 병목을 줄인 개선**으로 판정한다.
>
> ### 8.11. 완료 판정 (Step 8)
