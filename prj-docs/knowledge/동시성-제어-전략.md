...
### 2. 구현 방법 (How to Apply)
쿼리 시점에 DB의 배타 락(Exclusive Lock)을 직접 사용하도록 명시합니다.

```java
// [Repository]
@Lock(LockModeType.PESSIMISTIC_WRITE) // SELECT ... FOR UPDATE 실행
...
### 4. 실험 결과 및 분석
*   **성공 횟수**: 1건
*   **실패 횟수**: 29건 (애플리케이션 로직 `IllegalStateException`에서 차단)
*   **결론**: 비관적 락은 쿼리(`FOR UPDATE`)로 줄을 세워 정합성을 강력하게 보장한다. 하지만 대규모 트래픽 시 DB 부하가 우려되므로 Redis 분산 락 검토가 필요하다.

---
> **Next Work**: DB 부하 분산을 위한 **Redis 분산 락(Redisson)** 실험 예정.