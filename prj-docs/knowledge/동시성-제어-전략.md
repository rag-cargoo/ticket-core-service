...
> ###  4. 실제 구현 코드 (WaitingQueueServiceImpl)
> ---
> ```java
> @Service
> @RequiredArgsConstructor
...
> }
> ```
> ###  5. 검증 결과 및 효과 (Results & Impact)
> ---
>   - **실제 출력 로그 (`v5-waiting-queue.sh`)**:
> ```json
> [User 101] 대기열 진입 성공: {"userId":101,"concertId":1,"status":"WAITING","rank":1}
> [User 102] 대기열 진입 성공: {"userId":102,"concertId":1,"status":"WAITING","rank":2}
> ```
>   - **비포/애프터 비교**:
>   - **Before**: 요청 후 결과 수신 시까지 상태를 알 수 없어 불안 유발 및 무분별한 새로고침 발생.
>   - **After**: 실시간 순번 제공으로 사용자 심리적 안정 확보 및 서버 인입 트래픽 분산.
...
> ###  1. 개요 (Overview)
> ---
>   - 대기열 시스템의 완성은 단순히 줄을 세우는 것이 아니라, **서버와 DB가 감당 가능한 임계치만큼만 활성 상태로 전환하는 유입량 제어**에 있습니다.
> ### ️ 2. 핵심 제어 매커니즘 (Control Mechanisms)
> ---
>   - **2.1. 진입 차단 (Throttling)**: `ZCARD` 명령어로 현재 대기 인원을 체크하여 임계치 초과 시 진입 거부 응답 반환.
>   - **2.2. 유입량 동적 조절**: 서버 상태(CPU, DB 부하)에 따라 활성화 인원수를 가변적으로 운영.
>   - **2.3. ACTIVE 토큰 검증**: 인터셉터를 구현하여 Redis에 `active-user:{id}` 키가 존재하는 사용자만 예약 허용 (어뷰징 차단).
