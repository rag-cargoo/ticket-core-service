...
echo -e "${BLUE}>>>> [v2 Test] 비관적 락 검증 시작...${NC}"
...
USER_ID=$(curl $CURL_OPTS -X POST "$USER_API" -H "$CONTENT_TYPE" -d "{\"username\": \"test_v2_$(date +%s)\"}" | grep -oP '"id":\s*\K\d+' || true)
...
# 4. 예약 요청
echo -e "${YELLOW}[Step 3] 비관적 락 예약 API 호출 (Seat: $SEAT_ID)...${NC}"
RESPONSE=$(curl $CURL_OPTS -w "\n%{http_code}" -X POST "${BASE_URL}/v2/pessimistic" \
     -H "$CONTENT_TYPE" \
     -H "User-Id: ${USER_ID}" \
