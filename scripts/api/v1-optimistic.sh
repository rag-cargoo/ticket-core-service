...
SEAT_ID=$(curl -s "${CONCERT_API}/options/1/seats" | grep -oP '"id":\s*\K\d+' | head -n 1)
if [ -z "$SEAT_ID" ]; then
    SEAT_ID=$(curl -s "${CONCERT_API}/options/2/seats" | grep -oP '"id":\s*\K\d+' | head -n 1)
fi
...
USER_ID=$(curl $CURL_OPTS -X POST "$USER_API" -H "$CONTENT_TYPE" -d "{\"username\": \"test_$(date +%s)\"}" | grep -oP '"id":\s*\K\d+')
