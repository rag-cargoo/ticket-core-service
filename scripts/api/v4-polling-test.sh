...
curl -s -X POST "http://127.0.0.1:8080/api/reservations/v4-opt/queue-polling" -H "Content-Type: application/json" -d "{\"userId\": ${USER_ID}, \"seatId\": ${SEAT_ID}}"

echo -e "\n\n${YELLOW}[Step 2] Polling Status...${NC}"
for i in {1..5}
do
    STATUS=$(curl -s "http://127.0.0.1:8080/api/reservations/v4/status?userId=${USER_ID}&seatId=${SEAT_ID}")
